 ...
 *  Dependencies: Graph.java Edge.java Queue.java
...
public class KruskalMST {
    ...
    public KruskalMST(Graph G) {
        ...
        // comparator     
        class CompareLatency implements Comparator<Edge> {
            @Override
            public int compare(Edge o1, Edge o2) {
                if (o1.latency() > o2.latency())
                    return 1;
                else
                    return -1;
            }
        ...
        MinPQ<Edge> pq = new MinPQ<Edge>(new CompareLatency());
        for (Edge e : G.edges()) {
            pq.insert(e);
        }
    ...
    private boolean check(Graph G) {
        ...
        if (Math.abs(total - weight()) > FLOATING_POINT_EPSILON) {
            System.err.printf("Weight of edges does not equal weight(): %d vs. df\n", total, weight());
            return false;
        }
